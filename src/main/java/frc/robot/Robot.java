...
//super.autonomousInit();
  ...
  public void autonomousPeriodic() {
    ...
    double lwheelSpin = Math.abs(gRCombin) * Math.abs(s_mleft); 
    ...
    SmartDashboard.putNumber("State", AutoStage);
...
if (rwheelSpin > 50 & AutoRed == true) {
  if(rwheelSpin < 70 & AutoStage == 1){   //first turn right 45
    right.set(-0.3);
    left.set(0.3);
    m_collector.set(1);
  }
  else if(rwheelSpin > 71 & rwheelSpin < 151 & AutoStage == 1){
    left.set(-0.5);  //drive 7.07ft
      right.set(-0.5);
      m_collector.set(1);
    if(rwheelSpin > 146){
      AutoStage = 2;
    ...
    right.set(0.3); //turn 90 degrees left
    left.set(-0.3);
    m_collector.set(1);
    
    if (rwheelSpin < 132) {AutoStage = 3;} //sets it to the next auto stage

  }else if(rwheelSpin > 110 & rwheelSpin < 220 & AutoStage == 3){
    left.set(-0.5); //drives 7ft forward in auto stage
    right.set(-0.5);
    m_collector.set(1);
  } else if (rwheelSpin > 220 & rwheelSpin < 240 & AutoStage == 3) {
    right.set(-0.3); //turns 45 degrees right
    left.set(0.3);
    m_collector.set(1);
  } else if (rwheelSpin > 240 & rwheelSpin < 320 & AutoStage == 3) {
    right.set(-0.5);
    left.set(-0.5);
    m_collector.set(1);
...
} else if (rwheelSpin > 50 && AutoRed == false) {
  if(rwheelSpin < 80 & AutoStage == 1) {
    left.set(-0.4);  //drive 2.5ft
    right.set(-0.4);
  } else if (rwheelSpin > 79 & rwheelSpin < 95 & AutoStage == 1) {
    right.set(-0.3); //turns 45 degrees right
    left.set(0.3);
  } else if (rwheelSpin > 95 & rwheelSpin < 215 & AutoStage == 1) {
    left.set(-0.5);  //drive 7.07ft
    right.set(-0.5);
    m_collector.set(1);
    if(rwheelSpin > 180){ AutoStage = 2;}
  }else if(AutoStage == 2){
      if(rwheelSpin > 150 & rwheelSpin < 250){
      right.set(0.3); //turns 45 degrees right
      left.set(-0.3);
      if(rwheelSpin < 160){
        AutoStage = 3;
  ...
  /*if(rwheelSpin < 70 & AutoStage == 1){   //first turn right 45
    ...
    right.set(0.3); //turn 45 degrees left
    left.set(-0.3);
    m_collector.set(1);
    if (rwheelSpin < 145) {AutoStage = 3;} //sets it to the next auto stage

  } else if (rwheelSpin < 160 & AutoStage == 3) {
    left.set(-0.5);  //drive 2.5ft
      right.set(-0.5);
      m_collector.set(1);
      if (rwheelSpin > 155) {AutoStage = 4;} //sets it to the next auto stage

  } else if (AutoStage == 4) {
    right.set(0.3); //turn 45 degrees left
    left.set(-0.3);
    if (rwheelSpin < 151) {AutoStage = 5;} //sets it to the next auto stage
  } else if (AutoStage == 5 & rwheelSpin < 230) {
    left.set(-0.5);  //drive 7.07ft
    right.set(-0.5);
  }
*/
...
if(driveController.getRawAxis(3) > 0.7 && (NetworkTableInstance.getDefault().getTable("limelight").getEntry("tv").getDouble(0) == 1)) {
  ...
  if(tx < 5 && tx > -5){
      ...
      /*if(driveController.getY(Hand.kLeft) < -0.65){
        m_myRobot.arcadeDrive(driveController.getX(Hand.kRight), -0.65);

      }else{}*/
      m_myRobot.arcadeDrive(driveController.getX(Hand.kRight), -(driveController.getY(Hand.kLeft)));
      System.out.println(lwheelSpin);
